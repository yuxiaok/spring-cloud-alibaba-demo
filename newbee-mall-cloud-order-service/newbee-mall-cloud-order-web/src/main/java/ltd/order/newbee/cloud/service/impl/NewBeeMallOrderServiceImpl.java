...
import ltd.order.newbee.cloud.controller.vo.NewBeeMallOrderItemVO;
import ltd.order.newbee.cloud.controller.vo.NewBeeMallOrderListVO;
import ltd.order.newbee.cloud.dao.NewBeeMallOrderAddressMapper;
import ltd.order.newbee.cloud.dao.NewBeeMallOrderItemMapper;
import ltd.order.newbee.cloud.dao.NewBeeMallOrderMapper;
import ltd.order.newbee.cloud.entity.MallUserAddress;
import ltd.order.newbee.cloud.entity.NewBeeMallOrder;
import ltd.order.newbee.cloud.entity.NewBeeMallOrderAddress;
import ltd.order.newbee.cloud.entity.NewBeeMallOrderItem;
import ltd.order.newbee.cloud.service.NewBeeMallOrderService;
import ltd.shopcart.newbee.cloud.openfeign.NewBeeCloudShopCartServiceFeign;
...
public class NewBeeMallOrderServiceImpl implements NewBeeMallOrderService {
	...
	@Autowired
	private NewBeeCloudShopCartServiceFeign newBeeCloudShopCartServiceFeign;
	...
	public String saveOrder(Long userId, MallUserAddress address, List<Long> cartItemIds) {
		...
		//调用商品服务feign获取数据
		Result<List<NewBeeMallGoodsDTO>> goodsDTOListResult = newBeeCloudGoodsServiceFeign.getNewBeeMallGoodsByIds(goodsIds);
		...
		//判断商品库存
		for (NewBeeMallShoppingCartItemDTO shoppingCartItemDTO : itemsForSave) {
			//查出的商品中不存在购物车中的这条关联商品数据，直接返回错误提醒
			if (!newBeeMallGoodsMap.containsKey(shoppingCartItemDTO.getGoodsId())) {
				NewBeeMallException.fail(ServiceResultEnum.SHOPPING_ITEM_ERROR.getResult());
			}
			//存在数量大于库存的情况，直接返回错误提醒
			if (shoppingCartItemDTO.getGoodsCount() > newBeeMallGoodsMap.get(shoppingCartItemDTO.getGoodsId()).getStockNum()) {
		...
		if (!CollectionUtils.isEmpty(itemIdList) && !CollectionUtils.isEmpty(goodsIds) && !CollectionUtils.isEmpty(newBeeMallGoods)) {
			//调用购物车服务feign删除数据
			Result<Boolean> deleteByCartItemIdsResult = newBeeCloudShopCartServiceFeign.deleteByCartItemIds(itemIdList);
			if (deleteByCartItemIdsResult != null && deleteByCartItemIdsResult.getResultCode() == 200) {
				...
				updateStockNumDTO.setStockNumDTOList(stockNumDTOS);
				//调用商品服务feign修改库存数据
				Result<Boolean> updateStockResult = newBeeCloudGoodsServiceFeign.updateStock(updateStockNumDTO);
				...
				//总价
				for (NewBeeMallShoppingCartItemDTO newBeeMallShoppingCartItemDTO : itemsForSave) {
					priceTotal += newBeeMallShoppingCartItemDTO.getGoodsCount() * newBeeMallShoppingCartItemDTO.getSellingPrice();
				}
				if (priceTotal < 1) {
				...
				if (newBeeMallOrderMapper.insertSelective(newBeeMallOrder) > 0) {
					...
					for (NewBeeMallShoppingCartItemDTO newBeeMallShoppingCartItemDTO : itemsForSave) {
						...
						BeanUtil.copyProperties(newBeeMallShoppingCartItemDTO, newBeeMallOrderItem);
	...
	public Boolean recoverStockNum(List<Long> orderIds) {
		...
		//执行恢复库存的操作
		int updateStockNumResult = 2;
//		newBeeMallGoodsMapper.recoverStockNum(stockNumDTOS);
